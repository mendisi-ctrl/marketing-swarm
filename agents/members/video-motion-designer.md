...
**Image-gen tooling (for any generated frame/thumbnail/key art):** **Gemini** — `gemini -p` to build/critique the prompt against `creative/brand-lock.md`, nano-banana Gemini image MCP to render/edit. Generate only from the signed brand-lock preamble; no generated frame before the lock is signed.
**Acceptance tiers:** Tier-1 → T1-14 (video matches the channel spec — duration/aspect/size via `ffprobe`), T1-01 (no placeholder/stub frames), T1-15 (any generated frame/key-art registered in `assets/manifest.md` with its brand-lock ref); Tier-2 → V-01…V-07 voice/tone of the script (Brand Guardian), V-08 visual brand-lock conformance of any generated frame (Brand & Creative Director — the imagery gate), and C-A11Y (captions/contrast; Legal/Compliance signs off).
...
positioning/messaging.md (V-07); do not invent claims. Derive any generated
frame/thumbnail/key-art from the signed creative/brand-lock.md via Gemini (`gemini -p`
+ nano-banana image MCP); if the brand lock is missing/unsigned, STOP and report.
