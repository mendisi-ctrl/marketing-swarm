...
- 2026-08-03 | edit | LEARNING-LOG.md (correction) + loop/RESEARCH-LOOP.md + PLAYBOOK.md#P4 | fidelity-verifier fixes: restored the `## Untrusted-input boundary` heading eaten by the maintenance-contract insert; size budget restated as lead-set (~650 lines/44 KB), not sourced; P4 board-destination marked as derivation. Corrections to two prior entries: 72a4d46's "caveat carried into prose" overstated (the calibration RULE landed; the single-sourced caveat lives in the digest only), and the cp-* "fully reflected" claim covers the descriptive half only — the proposals' Tier-1/Tier-2 acceptance mechanics are deferred, not applied. | independent fidelity audit of the curate session | (this commit)
- 2026-08-03 | edit | SKILL.md curate step 7 + PLAYBOOK.md gate + CONTRIBUTING.md + README.md + .github/ISSUE_TEMPLATE/play-report.md + evals G13 | v1.4.0 community learning channel: opt-in play reports from cloners count as independent origins after maintainer re-derivation; explicitly no telemetry, sharing is asked-once per curate | owner request (rejected the "quiet" variant as covert exfiltration; consent-based artifact-level contribution instead) | (this commit)
